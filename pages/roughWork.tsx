...
const ChatSection = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col-reverse">
      {/* Chat messages */}
      <div className="flex-grow p-4 overflow-y-auto">
        {messages.map((message) => (
          <div key={message.id} className="p-2 mb-2 bg-white shadow rounded-lg">
            <p>{message.text}</p>
          </div>
        ))}
      </div>
      {/* Chat input area */}
      <form onSubmit={handleSubmit} className="w-full bg-white p-4 fixed bottom-0">
        <input
          type="text"
          value={inputText}
          onChange={(e) => setInputText(e.target.value)}
          className="w-full px-4 py-2 bg-gray-100 rounded-lg focus:outline-none focus:ring focus:ring-purple-400"
